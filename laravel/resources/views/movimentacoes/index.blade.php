            ...
            <div class="card-body">
                <form method="POST" action="{{ route('movimentacoes.store') }}">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="mb-3">
                                <label for="sobremesa_id" class="form-label">Sobremesa *</label>
                                <select class="form-select @error('sobremesa_id') is-invalid @enderror"
                                        id="sobremesa_id" name="sobremesa_id" required>
                                    <option value="">Selecione uma sobremesa...</option>
                                    @foreach($sobremesas as $sobremesa)
                                        <option value="{{ $sobremesa->id }}"
                                                data-estoque="{{ $sobremesa->estoque_atual }}"
                                                {{ old('sobremesa_id') == $sobremesa->id ? 'selected' : '' }}>
                                            {{ $sobremesa->nome }} (Estoque: {{ $sobremesa->estoque_atual }})
                                        </option>
                                    @endforeach
                                </select>
                                @error('sobremesa_id')
                        ...
                        <tbody>
                            @forelse($movimentacoes as $movimentacao)
                                <tr>
                                    ...
                                    <td><strong>{{ $movimentacao->sobremesa->nome }}</strong></td>
                                    <td>
...
document.addEventListener('DOMContentLoaded', function() {
    const sobremesaSelect = document.getElementById('sobremesa_id');
    const tipoSelect = document.getElementById('tipo');
    const quantidadeInput = document.getElementById('quantidade');

    function updateValidation() {
        const selectedOption = sobremesaSelect.options[sobremesaSelect.selectedIndex];
    ...
    sobremesaSelect.addEventListener('change', updateValidation);
