...
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="fas fa-birthday-cake"></i> Sobremesas</h2>
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#cadastrarModal">
                <i class="fas fa-plus"></i> Nova Sobremesa
            </button>
        </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @forelse($sobremesas as $sobremesa)
                                <tr>
                                    <td><strong>{{ $sobremesa->nome }}</strong></td>
                                    <td>{{ Str::limit($sobremesa->ingredientes, 50) }}</td>
                                    <td>R$ {{ number_format($sobremesa->preco, 2, ',', '.') }}</td>
                                    <td>
                                        <span class="badge bg-secondary">{{ $sobremesa->tamanho }}</span>
                                    </td>
                                    <td>{{ $sobremesa->categoria }}</td>
                                    <td>
                                        <span class="badge {{ $sobremesa->estoque_baixo ? 'bg-danger' : 'bg-success' }}">
                                            {{ $sobremesa->estoque_atual }}
                                        </span>
                                    </td>
                                    <td>{{ $sobremesa->estoque_minimo }}</td>
                                    <td>
                                        @if($sobremesa->ativo)
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <button class="btn btn-outline-primary"
                                                    ...
                                                    data-sobremesa='@json($sobremesa)'>
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <form method="POST" action="{{ route('sobremesas.destroy', $sobremesa) }}"
                                                  class="d-inline"
                                                  onsubmit="return confirm('Tem certeza que deseja excluir esta sobremesa?')">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="cadastrarModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" action="{{ route('sobremesas.store') }}">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-plus"></i> Nova Sobremesa</h5>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" id="editarForm">
                ...
                @method('PUT')
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-edit"></i> Editar Sobremesa</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('editarModal').addEventListener('show.bs.modal', function (event) {
        ...
        const sobremesa = JSON.parse(button.getAttribute('data-sobremesa'));

        document.getElementById('editarForm').action = `/sobremesas/${sobremesa.id}`;
        document.getElementById('edit_nome').value = sobremesa.nome;
        document.getElementById('edit_preco').value = sobremesa.preco;
        document.getElementById('edit_tamanho').value = sobremesa.tamanho;
        document.getElementById('edit_categoria').value = sobremesa.categoria;
        document.getElementById('edit_ingredientes').value = sobremesa.ingredientes;
        document.getElementById('edit_estoque_minimo').value = sobremesa.estoque_minimo;
        document.getElementById('edit_ativo').checked = sobremesa.ativo;
    });
});
</script>
@endsection
